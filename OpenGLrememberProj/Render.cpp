...
void Floor(double h, double x)
{
	...
	glBegin(GL_TRIANGLES);
	for (size_t i = 0; i < deCount - 1; i++)
	{
		glVertex3d(deX[i], deY[i], h);
		glVertex3d(deX[i + 1], deY[i + 1], h);
		glVertex3d(-4.25, 0.75, h);
	}
	for (size_t i = 0; i < dcCount - 1; i++)
	{
		glVertex3d(dcX[i], dcY[i], h);
		glVertex3d(dcX[i + 1], dcY[i + 1], h);
		glVertex3dv(O);
	}
	glEnd();
}
...
void Wall(double z)
{
	...
	for (size_t i = 0; i < deCount - 1; i++)
	{
		...
		glVertex3d(deX[i], deY[i], 0);
		glVertex3d(deX[i], deY[i], h);
		glVertex3d(deX[i + 1], deY[i + 1], h);
		glVertex3d(deX[i + 1], deY[i + 1], 0);
	}
	glEnd();
	...
	for (size_t i = 0; i < dcCount - 1; i++)
	{
		...
		d[1] = deY[i + 1];
		d[2] = 0;
		glNormal3dv(normal(c, a, b));
		glVertex3d(dcX[i], dcY[i], 0);
		glVertex3d(dcX[i], dcY[i], h);
		glVertex3d(dcX[i + 1], dcY[i + 1], h);
		glVertex3d(dcX[i + 1], dcY[i + 1], 0);
	}
	glEnd();
}
